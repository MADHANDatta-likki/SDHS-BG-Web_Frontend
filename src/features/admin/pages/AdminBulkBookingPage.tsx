...
import type { BulkBookingEntry, BulkBookingTrackType } from "../types/api";
...
function AdminBulkBookingPage() {
  ...
  const [trackType, setTrackType] = useState<BulkBookingTrackType | "">("");
  ...
  useEffect(() => { if (trackType === "REVISION" || !student || !chapter) return setAllowed([]); void adminService.getAllowedSlokas({ volunteerId: student, date, chapterId: Number(chapter) }).then((r) => setAllowed(r.allowed)).catch(() => setAllowed([])); }, [student, date, chapter, trackType]);
  useEffect(() => { if (trackType === "REVISION" || !student || !chapter2) return setAllowed2([]); void adminService.getAllowedSlokas({ volunteerId: student, date, chapterId: Number(chapter2) }).then((r) => setAllowed2(r.allowed)).catch(() => setAllowed2([])); }, [student, date, chapter2, trackType]);
  const stage = () => { if (!trackType) return setNotice({ type: "error", text: "Track Type is required." }); const revision = trackType === "REVISION"; if (!student || !slot || !chapter || (!revision && !count)) return setNotice({ type: "error", text: revision ? "Student, slot, and chapter are required." : "Student, slot, chapter, and sloka count are required." }); if (!revision && allowed.length > 0 && !allowed.includes(Number(count))) return setNotice({ type: "error", text: `Allowed sloka counts: ${allowed.join(", ")}.` }); if (second && (!chapter2 || (!revision && !count2))) return setNotice({ type: "error", text: revision ? "Second chapter is required." : "Second chapter and sloka count are required." }); const entry: BulkBookingEntry = { volunteerId: student, date, slotId: Number(slot), chapterId: Number(chapter), ...(!revision ? { slokaCount: Number(count) } : {}), ...(second ? { chapterId2: Number(chapter2), ...(!revision ? { slokaCount2: Number(count2) } : {}) } : {}) }; setEntries((current) => [...current, entry]); setNotice(null); };
  const save = async () => { if (entries.length === 0) return; if (!trackType) return setNotice({ type: "error", text: "Track Type is required." }); setWorking(true); setNotice(null); try { const response = await adminService.saveBulkBooking({ trackType, entries }); const detail = response.messages.length ? ` ${response.messages.join(" ")}` : ""; setNotice({ type: response.failed ? "error" : "success", text: `${response.saved} saved, ${response.failed} failed.${detail}` }); if (!response.failed) setEntries([]); await reload(); } catch (e: unknown) { setNotice({ type: "error", text: getAdminApiError(e, "Failed to save bookings.") }); } finally { setWorking(false); } };
        ...
        <>
          <AdminCard title="Add Booking">
            <div className="admin-form-grid">
              <label className="admin-field">Track Type<select required value={trackType} onChange={(event) => { setTrackType(event.target.value as BulkBookingTrackType | ""); setEntries([]); }}><option value="">Select track type</option><option value="MEMORIZATION">Memorization</option><option value="REVISION">Revision</option></select></label>
              <AdminStudentAutocomplete selectedVolunteerId={student} onSelect={(selectedStudent) => setStudent(selectedStudent.volunteerId)} />
              ...
              {trackType === "REVISION" ? <div className="admin-field"><span>Sloka Range</span><strong>Whole Chapter</strong></div> : <label className="admin-field">Sloka Count<select value={count} onChange={(event) => setCount(event.target.value)}><option value="">Select count</option>{allowed.map((value) => <option key={value}>{value}</option>)}</select></label>}
              ...
              {second && <><label className="admin-field">Second Chapter<select value={chapter2} onChange={(event) => setChapter2(event.target.value)}><option value="">Select chapter</option>{data.chapters.map((item) => <option key={item.id} value={item.id}>{formatChapterLabel(item.chapterNumber, item.chapterName, "Ch")}</option>)}</select></label>{trackType === "REVISION" ? <div className="admin-field"><span>Second Sloka Range</span><strong>Whole Chapter</strong></div> : <label className="admin-field">Second Sloka Count<select value={count2} onChange={(event) => setCount2(event.target.value)}><option value="">Select count</option>{allowed2.map((value) => <option key={value}>{value}</option>)}</select></label>}</>}
            </div>
